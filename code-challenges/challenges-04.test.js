...
const isCapitalized = (str) => {
  // Solution code here...
  const regexToTest = /[A-Z]\w+/gm;
  return str.match(regexToTest) || []
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  const result = [];
  const regexToTest = /^[A-J]\w+\b;
  arr.forEach(testRegex => {
    if(regexToTest.test(testRegex)) {
        result.push(testRegex)
    }
  })
  return result
}
